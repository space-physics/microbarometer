...
loc = {
    'TA-BGNE-BDO': (-98.150200, 41.408298, 573.000000, 'Belgrade, Nebraska'),  # fs= 40 Hz
    'IU-CCM-LDO': (-91.244598, 38.055698, 222.000000, 'Cathedral Cave, Missouri'),
}  # fs= 1 Hz
...
def plotspecgram(dat, fs: int, flim: tuple, clim: tuple):
    ...
    if 1:
        ...
        f, t, Sxx = scipy.signal.spectrogram(dat.data, fs=fs, nperseg=Nperseg)  # ,
        #  nfft= Nfft,

        #  noverlap= None) # [V**2/Hz]
        step = timedelta(seconds=t[1] - t[0])
        t = pmb.datetimerange(dat.meta.starttime.datetime, dat.meta.endtime.datetime + step, step)
